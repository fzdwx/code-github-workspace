...
const (
	HeaderStyleKey tableStyleKey = iota
	FooterStyleKey
	RowsStyleKey
	RowsSubsequentStyleKey
	RowsCursorStyleKey
	CellCursorStyleKey
)
...
// BadTypeError type does not match Ordered interface types
...
// RowLenError row length is not matching headers len
...
// BadCellTypeError type of cell does not match type of column
type BadCellTypeError struct {
	msg string
}

func (e BadCellTypeError) Error() string {
	return e.msg
}
...
// New initialize Table object with defaults
func New(width, height int, columnHeaders []string) *Table {
	...
	styles := map[tableStyleKey]lipgloss.Style{
		HeaderStyleKey:         tableDefaultHeaderStyle,
		FooterStyleKey:         tableDefaultFooterStyle,
		RowsStyleKey:           tableDefaultRowsStyle,
		RowsSubsequentStyleKey: tableDefaultRowsSubsequentStyle,
		RowsCursorStyleKey:     tableDefaultRowsCursorStyle,
		CellCursorStyleKey:     tableDefaultCellCursorStyle,
	}
	...
	r := &Table{
		...
		orderedColumnPhase: SortingDescending,
...
func (r *Table) SetTypes(columnTypes ...any) (*Table, error) {
	...
	for i, t := range columnTypes {
		if !isOrdered(t) {
			...
			return r, BadTypeError{msg: message}
		}
	}
...
func (r *Table) Render() string {
	...
	return lipgloss.JoinVertical(
		...
		r.headerBox.Render(),
		r.rowsBox.Render(),
		r.styles[FooterStyleKey].Width(r.width).Render(statusMessage),
	)
}
...
func (r *Table) validateRow(cells ...any) error {
	...
	if len(cells) != len(r.columnType) {
		...
		return RowLenError{msg: message}
	}
	// check cell type
	for i, c := range cells {
		switch c.(type) {
...
func (r *Table) updateHeader() *Table {
	...
	r.headerBox.SetStyle(r.styles[HeaderStyleKey])
	for i, title := range r.columnHeaders {
		...
		if e {
			...
			if r.orderedColumnIndex == i {
				if r.orderedColumnPhase == SortingDescending {
					titleSuffix = " " + tableDefaultSortDescChar
				} else if r.orderedColumnPhase == SortingAscending {
	...
	for ir, columns := range r.filteredRows[r.rowsTopIndex:rowsBottomIndex] {
		...
		for ic, column := range columns {
			...
			if irCorrected == r.cursorIndexY && ic == r.cursorIndexX {
				c.SetStyle(r.styles[CellCursorStyleKey])
			}
		...
		if irCorrected == r.cursorIndexY {
			rw.SetStyle(r.styles[RowsCursorStyleKey])
		} else if irCorrected%2 == 0 || irCorrected == 0 {
			rw.SetStyle(r.styles[RowsSubsequentStyleKey])
		} else {
			rw.SetStyle(r.styles[RowsStyleKey])
		}
...
func (r *Table) updateOrderedVars(index int) {
	...
	if r.orderedColumnIndex == index {
		...
		case SortingAscending:
			r.orderedColumnPhase = SortingDescending

		case SortingDescending:
			r.orderedColumnPhase = SortingAscending
		}
	} else {
		r.orderedColumnPhase = SortingDescending
	}
...
func sortIndex[T Ordered](slice []T, order SortingOrderKey) []int {
	...
	for i := len(slice); i > 0; i-- {
		for j := 1; j < i; j++ {
			if order == SortingDescending && slice[j] < slice[j-1] {
				slice[j], slice[j-1] = slice[j-1], slice[j]
				index[j], index[j-1] = index[j-1], index[j]
			} else if order == SortingAscending && slice[j] > slice[j-1] {
				slice[j], slice[j-1] = slice[j-1], slice[j]
				index[j], index[j-1] = index[j-1], index[j]
			}
		}
